...
void readCSC(CSC* myCSC, FILE* file){
    ...
    while(fscanf(file, "%d %d %lf", &row, &col, &entry) > 0){
    	...
    	//myCSC->values[idx] = entry;
        //printf("Copying %d into r_indices array\n", row);
        //myCSC->r_indices[idx++] = row-1;
        arr[row-1][col-1] = entry;
        //printf("Copying %d into c_offset array\n", col);
        myCSC->c_offset[col]++;
    }
...
double mulSequential(CSC* myCSC){
